...
紙とインクだけで成立する Editorial / Magazine-inspired design。色に頼らず、セリフ体の見出しとサンセリフ体の本文の対比、ウェイトとサイズの段階、そして余白の量と質で情報階層を表現する。方眼紙のグリッド背景が「紙上の分析ノート」という世界観を演出する。

| # | Principle | 説明 |
|---|-----------|------|
| 1 | Paper-First | 紙とインクだけで成立するデザイン。色に頼らない |
| 2 | Monochrome Discipline | 使用色は黒・白・グレーのみ |
...
- ヒーロー `display-xl` light + italic。ヘッダー直下に `border-heavy`
- セクション間 `space-9`+。CTA は `btn-primary`
- 背景にグリッドパターン

### Web App

- サーフェス階層: `bg-page` → `bg-surface` → `bg-surface-raised`
- サイドバー `paper-kraft`。ヘッダー `backdrop-filter: blur(8px)`
- 本文 `body-md`、密度の高いUIは `body-sm`

### Mobile (iOS)

- iOS HIG 優先。カラートークンのみ適用
- タッチターゲット 44×44pt 以上
- SF Pro 基本、見出しのみ Crimson Pro 検討。Dynamic Type サポート

### Presentation (pptx)

AI がスライドデッキ（pptx）を生成する際に従うルールセット。CSS ブロックは視覚仕様のリファレンスとして記載している。pptx 出力時は同等の値をスライドプロパティに変換して適用する。カラーパレット・セマンティックトークンは「Color Tokens」セクションを参照。ボーダーラウンドは「Shape & Lines」セクションを参照。

#### 基本設定

- 出力形式: pptx（PowerPoint）
- サイズ: 1920×1080px（16:9） — pptx 換算 13.333" × 7.5"
- フォント: Crimson Pro (400,700) + Instrument Sans (500,700) + JetBrains Mono (500,600)

#### タイポグラフィ

**注意: スライドでは 10px 未満を使わない。**

テキストカラーはタイポグラフィでは固定しない。セマンティックトークン（text-heading, text-primary, text-secondary, text-muted 等）を背景色との組み合わせで選択する。具体的な対応は「Color Tokens」セクションの使い分け表を参照のこと。
...
**枠線と中塗りの使い分け** — 枠線（border/stroke）と中塗り（background fill）は基本的に併用しない。**併用可能な例外**: 中塗りがページ背景色に十分近い薄さで、同系統のグレーで枠線を引く場合のみ許可（例: Flat Paper の paper-cream 背景 + ink-100 ボーダー）
